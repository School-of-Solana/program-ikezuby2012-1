...
pub fn process_init_reserve(ctx: Context<InitReserve>, liquidation_threshold: u64, max_ltv: u64) -> Result<()> {
    let reserve = &mut ctx.accounts.reserve;
    reserve.mint_address = ctx.accounts.mint.key();
    reserve.authority = ctx.accounts.signer.key();
    reserve.liquidation_threshold = liquidation_threshold;
    reserve.max_ltv = max_ltv;
